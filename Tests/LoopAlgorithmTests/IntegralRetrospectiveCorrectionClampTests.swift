...
//  the LoopAlgorithm port and restored 2026-06-19. The clamp bounds the wound-up
//  integral term by an ISF×basal-scaled, target-relative window so IRC can't
//  drive the forecast into over-/under-dosing. When the clamp inputs (ISF, basal
//  rate, correction range) are nil, IRC must behave exactly as before (legacy
//  unclamped), so prior IRC experiments reproduce byte-identical.
//
