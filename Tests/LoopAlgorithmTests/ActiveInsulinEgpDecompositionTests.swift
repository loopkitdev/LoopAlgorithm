...
//  default — no `scheduleBaselineSensitivityHistory` argument) the output is bit-
...
final class ActiveInsulinEgpDecompositionTests: XCTestCase {
        ...
        for dose in doses {
            for tau in [TimeInterval(.minutes(15)), .minutes(60), .minutes(180), dia, dia + .hours(2)] {
                ...
                let decomp = dose.glucoseEffect(at: t,
                                                ...
                                                scheduleBaselineSensitivity: 50.0,
                                                delta: .minutes(5))
    ...
    func testActiveSensitivityBoostScalesOnlyBolusEffect() {
        ...
        let baseline = bolus.glucoseEffect(at: evalT, activeSensitivity: 50, scheduleBaselineSensitivity: 50, delta: .minutes(5))
        let activeDoubled = bolus.glucoseEffect(at: evalT, activeSensitivity: 100, scheduleBaselineSensitivity: 50, delta: .minutes(5))
    ...
    func testEgpSensitivityDoesNotAffectBolus() {
        ...
        let bolus = BasalRelativeDose(type: .bolus, startDate: start, endDate: start, volume: 1.0)
        let evalT = start.addingTimeInterval(dia)
        let base = bolus.glucoseEffect(at: evalT, activeSensitivity: 50, scheduleBaselineSensitivity: 50, delta: .minutes(5))
        let egpDoubled = bolus.glucoseEffect(at: evalT, activeSensitivity: 50, scheduleBaselineSensitivity: 100, delta: .minutes(5))
    ...
    func testEgpSensitivityScalesOnlySuspendEffect() {
        ...
        let base = suspend.glucoseEffect(at: evalT, activeSensitivity: 50, scheduleBaselineSensitivity: 50, delta: .minutes(5))
        let activeDoubled = suspend.glucoseEffect(at: evalT, activeSensitivity: 100, scheduleBaselineSensitivity: 50, delta: .minutes(5))
        XCTAssertEqual(base, activeDoubled, accuracy: 1e-12,
            "Active sensitivity must NOT affect a suspended-temp (netBasalUnits < 0)")

        let egpDoubled = suspend.glucoseEffect(at: evalT, activeSensitivity: 50, scheduleBaselineSensitivity: 100, delta: .minutes(5))
        ...
        for active in [10.0, 50.0, 100.0] {
            for egp in [10.0, 50.0, 100.0] {
                let v = neutral.glucoseEffect(at: evalT,
                                              ...
                                              scheduleBaselineSensitivity: egp,
                                              delta: .minutes(5))
    ...
    func testCollectionLevelDecomposedMatchesSingleWhenSchedulesEqual() {
        ...
        let decompNilEgp = doses.glucoseEffects(insulinSensitivityHistory: isf, scheduleBaselineSensitivityHistory: nil)
        let decompMatching = doses.glucoseEffects(insulinSensitivityHistory: isf, scheduleBaselineSensitivityHistory: isf)
    ...
    func testCollectionLevelOverrideBoostsOnlyActiveComponent() {
        ...
        let decomposed = [bolus, suspend].glucoseEffects(
            insulinSensitivityHistory: boostedISF,
            scheduleBaselineSensitivityHistory: baseISF,
            from: start, to: endTime
        )
    ...
    func testMidAbsorptionDecomposedMatchesSingleWhenSchedulesEqual() {
        ...
        let decomp = doses.glucoseEffectsMidAbsorptionISF(insulinSensitivityHistory: isf, scheduleBaselineSensitivityHistory: isf)
        let decompNil = doses.glucoseEffectsMidAbsorptionISF(insulinSensitivityHistory: isf, scheduleBaselineSensitivityHistory: nil)
    ...
    /// `generatePrediction(..., scheduleBaselineSensitivity: nil)` must produce a
    ...
    func testGeneratePredictionNilEgpMatchesSingleSchedule() {
        ...
        let predA = LoopAlgorithm.generatePrediction(
            ...
            basal: basal, sensitivity: sensitivity,
            scheduleBaselineSensitivity: nil,
            carbRatio: carbRatio,
        ...
        let predB = LoopAlgorithm.generatePrediction(
            ...
            carbEntries: [FixtureCarbEntry](),
            basal: basal, sensitivity: sensitivity,
            scheduleBaselineSensitivity: sensitivity,  // same as insulin → must match nil-case
            carbRatio: carbRatio,
            useMidAbsorptionISF: true
        )
