...
extension BasalRelativeDose {
    ...
    ///
    /// At `activeSensitivity == scheduleBaselineSensitivity` (the default in the single-
    ...
    /// `activeSensitivity` but leave `scheduleBaselineSensitivity` at scheduled — otherwise
...
extension Collection where Element == BasalRelativeDose {
    ...
    /// `scheduleBaselineSensitivityHistory` (when non-nil) to the EGP-credit (negative
    /// `netBasalUnits`) component. When `scheduleBaselineSensitivityHistory` is nil,
    ...
    /// `scheduleBaselineSensitivityHistory` that reflects only the EGP-rate change
    ...
    public func glucoseEffects(
        ...
        repeat {
            let value = reduce(0) { (value, dose) -> Double in
                ...
                let egpISF: Double
                if let egp = scheduleBaselineSensitivityHistory,
                ...
                return value + dose.glucoseEffect(at: date,
                                                  ...
                                                  scheduleBaselineSensitivity: egpISF,
                                                  delta: delta)
            }
    ...
    /// ISF overload but separately accepts `scheduleBaselineSensitivityHistory` for the
    /// EGP-credit (negative `netBasalUnits`) component. When
    /// `scheduleBaselineSensitivityHistory` is nil, behavior is identical to the single-
    /// ISF overload.
    ///
    /// Assumes `scheduleBaselineSensitivityHistory` (when non-nil) has the same time
